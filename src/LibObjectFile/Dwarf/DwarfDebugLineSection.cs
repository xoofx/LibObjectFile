...
namespace LibObjectFile.Dwarf
{
    ...
    public sealed class DwarfDebugLineSection : DwarfSection
    {
        ...
        {
            if (reader.InputOutputContext.DebugLineStream.Stream == null)
            {
                return;
            }
            ...
            try
            {
                reader.Stream = reader.InputOutputContext.DebugLineStream;
                ReadInternal(reader, reader.InputOutputContext.DebugLineStream.RawDump);
            }
            finally
            {
        ...
        private void ReadInternal(DwarfReaderWriter reader, TextWriter rawDump)
        {
            ...
            IsTarget64Bit = reader.IsTargetAddress64Bit;
        ...
        public override bool TryUpdateLayout(DiagnosticBag diagnostics)
        {
            ...
            ulong sizeOf = DwarfHelper.SizeOfUnitLength(Is64Bit);
            // unit_length
            sizeOf += DwarfHelper.SizeOfNativeInt(Is64Bit);
        ...
        internal void Write(DwarfReaderWriter writer)
        {
            ...
            try
            {
                writer.Stream = writer.InputOutputContext.DebugLineStream;
                Write(writer, writer.InputOutputContext.DebugLineStream.RawDump);
            }
            finally
            {
        ...
        private void Write(DwarfReaderWriter writer, TextWriter rawDump)
        {
            ...
            writer.Is64Bit = Is64Bit;
            writer.WriteUnitLength(Size - DwarfHelper.SizeOfUnitLength(Is64Bit));
            
            writer.WriteU16(Version);
            writer.WriteNativeUInt(HeaderLength);
            ...
            {
                writer.WriteLEB128(opcodeLength);
            }
            ...
            foreach (var fileName in FileNames)
            {
                ...
                writer.WriteLEB128(directoryIndex);
                writer.WriteLEB128(fileName.Time);
                writer.WriteLEB128(fileName.Size);
            }
            // empty string
            writer.WriteU8(0);
        ...
        private void WriteDebugLineOpCodes(DwarfReaderWriter writer, uint opCodeBase)
        {
            ...
            foreach (var debugLine in Lines)
            {
                ...
                if (deltaColumn != 0)
                {
                    ...
                    writer.WriteLEB128(debugLine.Column);
                }
                ...
                if (fileNameChanged)
                {
                    ...
                    else
                    {
                        ...
                        writer.WriteLEB128(sizeOfInlineFileName);

                        writer.WriteU8(DwarfNative.DW_LNE_define_file);
                        writer.WriteStringUTF8NullTerminated(fileName.Name);
                        writer.WriteLEB128(dirIndex);
                        writer.WriteLEB128(fileName.Time);
                        writer.WriteLEB128(fileName.Size);
                    }
                }
                ...
                if (isaChanged)
                {
                    ...
                    writer.WriteLEB128(debugLine.Isa);
                }
                ...
                {
                    writer.WriteU8(0);
                    writer.WriteLEB128(1 + DwarfHelper.SizeOfLEB128(debugLine.Discriminator));
                    writer.WriteU8(DwarfNative.DW_LNE_set_discriminator);
                    writer.WriteLEB128(debugLine.Discriminator);
                }
                ...
                if (!canEncodeLineInSpecialCode)
                {
                    if (deltaLine != 0)
                    {
                        ...
                        writer.WriteSignedLEB128(deltaLine);
                        deltaLine = 0;
                    }
                }
                ...
                if (canEncodeLineInSpecialCode && (deltaAddress > 0 || deltaOperationIndex > 0))
                {
                    ...
                    else
                    {
                        ...
                        {
                            writer.WriteU8(0);
                            writer.WriteLEB128(1 + DwarfHelper.SizeOfNativeInt(writer.IsTargetAddress64Bit));
                            writer.WriteU8(DwarfNative.DW_LNE_set_address);
                            writer.WriteTargetUInt(debugLine.Address);
                            operationAdvancedEncoded = true;
                        }
                ...
                if (!operationAdvancedEncoded)
                {
                    if (deltaAddress > 0 || deltaOperationIndex > 0)
                    {
                        ...
                        writer.WriteLEB128(operation_advance);
                    }
                }
            ...
            foreach (var debugLine in Lines)
            {
                ...
                if (canEncodeLineInSpecialCode && (deltaAddress > 0 || deltaOperationIndex > 0))
                {
                    ...
                    else
                    {
                        ...
                        {
                            sizeOf += 1; // writer.WriteU8(0);
                            var sizeOfAddress = DwarfHelper.SizeOfNativeInt(IsTarget64Bit);
        ...
        {
            return $"Section .debug_line, {nameof(Version)}: {Version}, {nameof(Is64Bit)}: {Is64Bit}, {nameof(FileNames)}: {FileNames.Count}, {nameof(Lines)}: {Lines.Count}";
        }
    }
}