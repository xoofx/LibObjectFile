...
{
    public class DwarfDebugLine : ObjectFileNode<DwarfDebugLineSection>
    {
        // -----------------------
        // DWARF 2
        // -----------------------
        ...
        /// </summary>
        public DwarfDebugFileName File { get; set; }
        ...
        public DwarfDebugLine Clone()
        {
            return (DwarfDebugLine) MemberwiseClone();
        }

        internal void Delta(DwarfDebugLine against, out ulong deltaAddress,
        ...
        internal void Reset(DwarfDebugFileName firstFile, bool isStatement)
        {
            Address = 0;
            File = firstFile;
            Line = 1;
        ...
        internal DwarfDebugLineState ToState()
        {
            return new DwarfDebugLineState()
            {
                Address = Address,
                Column = Column,
    ...
    internal struct DwarfDebugLineState
    {
        // -----------------------
        // DWARF 2
        // -----------------------
        ...
        internal void Delta(in DwarfDebugLineState against, 
            out ulong deltaAddress,
            out int deltaOperationIndex,
            out bool fileNameChanged,
            out int deltaLine,
            out int deltaColumn,
            out bool isStatementChanged,
        ...
        {
            deltaAddress = against.Address - this.Address;
            deltaOperationIndex = against.OperationIndex - this.OperationIndex;
            fileNameChanged = !ReferenceEquals(this.File, against.File);
            deltaLine = (int)((long)against.Line - (long)this.Line);
            deltaColumn = (int)((long)against.Column - (long)this.Column);
            isStatementChanged = against.IsStatement != this.IsStatement;
        ...
        public override string ToString()
        {
            return $"{nameof(Address)}: 0x{Address:x16}, {nameof(File)}: {File}, {nameof(Line)}: {Line,4}, {nameof(Column)}: {Column,2}, {nameof(IsStatement)}: {Bool2Str(IsStatement),5}, {nameof(IsBasicBlock)}: {Bool2Str(IsBasicBlock),5}, {nameof(IsEndSequence)}: {Bool2Str(IsEndSequence),5}, {nameof(IsPrologueEnd)}: {Bool2Str(IsPrologueEnd),5}, {nameof(IsEpilogueBegin)}: {Bool2Str(IsEpilogueBegin),5}, {nameof(Isa)}: {Isa,3}, {nameof(Discriminator)}: {Discriminator,3}";
        }
